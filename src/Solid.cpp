...
void Solid::fillTriangles(std::vector<GLfloat> &vertices,
	...
	solidsortx = ArrayVector2D<int>(polygon.size());
	solidsorty = ArrayVector2D<int>(polygon.size());
	solidsortz = ArrayVector2D<int>(polygon.size());
	solidsort = ArrayVector2D<int>(polygon.size());
	aux = std::vector<int>(polygon.size());
...
void Solid::moveSphere(glm::vec4 v) {
	bs.pos += v;
}

void Solid::setSpherePos(glm::vec4 v) {
	bs.pos = v;
}

std::vector<triangle>& Solid::getPolygon() {
	return polygon;
}

ArrayVector2D<int>& Solid::getSolidSortX() {
	return solidsortx;
}

ArrayVector2D<int>& Solid::getSolidSortY() {
	return solidsorty;
}

ArrayVector2D<int>& Solid::getSolidSortZ() {
	return solidsortz;
}

ArrayVector2D<int>& Solid::getSolidSort() {
	return solidsort;
}

std::vector<int>& Solid::getAux() {
	return aux;
}

void Solid::updateVertices() {
	auto mat = objectspaceTrans
